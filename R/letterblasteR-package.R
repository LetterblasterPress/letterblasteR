#' @keywords internal
"_PACKAGE"

## usethis namespace: start
#' @importFrom cli ansi_html
#' @importFrom cli ansi_html_style
#' @importFrom covr coverage_to_list
#' @importFrom covr file_report
#' @importFrom covr package_coverage
#' @importFrom devtools check
#' @importFrom devtools document
#' @importFrom dplyr across
#' @importFrom dplyr arrange
#' @importFrom dplyr coalesce
#' @importFrom dplyr desc
#' @importFrom dplyr distinct
#' @importFrom dplyr filter
#' @importFrom dplyr if_else
#' @importFrom dplyr left_join
#' @importFrom dplyr mutate
#' @importFrom dplyr transmute
#' @importFrom forcats fct_relabel
#' @importFrom forcats fct_rev
#' @importFrom fs dir_copy
#' @importFrom fs dir_create
#' @importFrom fs dir_exists
#' @importFrom fs dir_ls
#' @importFrom fs file_copy
#' @importFrom fs file_exists
#' @importFrom fs file_move
#' @importFrom fs file_temp
#' @importFrom fs path
#' @importFrom fs path_dir
#' @importFrom fs path_file
#' @importFrom fs path_temp
#' @importFrom gert git_add
#' @importFrom gert git_commit
#' @importFrom gert git_config_set
#' @importFrom gert git_init
#' @importFrom ggplot2 aes
#' @importFrom ggplot2 element_blank
#' @importFrom ggplot2 element_rect
#' @importFrom ggplot2 facet_grid
#' @importFrom ggplot2 geom_rect
#' @importFrom ggplot2 ggplot
#' @importFrom ggplot2 labs
#' @importFrom ggplot2 scale_fill_manual
#' @importFrom ggplot2 theme
#' @importFrom grid unit
#' @importFrom htmltools HTML
#' @importFrom htmltools html_print
#' @importFrom htmltools tagList
#' @importFrom htmltools tags
#' @importFrom magrittr %<>%
#' @importFrom methods hasArg
#' @importFrom methods is
#' @importFrom pandoc pandoc_convert
#' @importFrom pkgdown build_site
#' @importFrom purrr map
#' @importFrom purrr map_chr
#' @importFrom purrr map_dbl
#' @importFrom purrr map_dfc
#' @importFrom purrr map_lgl
#' @importFrom purrr map_vec
#' @importFrom rstudioapi viewer
#' @importFrom styler style_pkg
#' @importFrom tibble tibble
#' @importFrom tidyr pivot_longer
#' @importFrom tidyr pivot_wider
#' @importFrom tidyr separate
#' @importFrom tidyselect ends_with
#' @importFrom tidyselect matches
#' @importFrom usethis use_r
#' @importFrom usethis use_test
#' @importFrom usethis use_tidy_description
#' @importFrom utils capture.output
#' @importFrom utils download.file
#' @importFrom utils head
#' @importFrom utils unzip
#' @importFrom withr with_dir
#' @importFrom yaml read_yaml
## usethis namespace: end
NULL

utils::globalVariables(".data")
utils::globalVariables("ratios")
utils::globalVariables("layouts")
utils::globalVariables("page_height")
utils::globalVariables("page_width")
utils::globalVariables("page_ratio")
utils::globalVariables("text_height")
utils::globalVariables("text_width")
utils::globalVariables("text_ratio")
utils::globalVariables("text_pct")
utils::globalVariables("b_mar")
utils::globalVariables("t_mar")
utils::globalVariables("o_mar")
utils::globalVariables("i_mar")
